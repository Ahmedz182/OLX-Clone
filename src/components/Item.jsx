...
const Item = ({ title, price, img, location, time, id, categories }) => {
  const [Like, setLike] = useState(false);
  const [Load, setLoad] = useState(false);
  const navigate = useNavigate();

  const onClickHandler = () => navigate(`/${categories}/${id}`);
    ...
    <>
      ...
      {Load && (
        <div
          ...
          className=" outline outline-1 flex flex-col outline-text/20 gap-3  rounded w-[23%] sm:w-[33%] md:w-[30%] cursor-pointer">
          <img
            src={img}
            className=" imgItem "
            loading="lazy"
